    ...
    <div class="graph-visual tables-main">
        <h4 class="inner-tittle">
            <div class="share share_size_large share_type_twitter">
                <a class="share__btn" href="#" data-toggle="modal" data-target="#createModal">新增部门</a>
...
<script type="text/javascript">
    ...
    function flushPage() {
        <!--初始化列表-->
        $.ajax({
            url : "http://localhost:8001/dept/queryPage",
    ...
    function buildList(list) {
        //清空列表
        $("#tableList").empty();
        var headCont = "<thead><tr><th>id</th> <th>部门名称</th> <th>上级部门id</th> <th>部门层级</th> <th>部门在当前层级下的顺序</th><th>操作</th></tr></thead>";
        ...
        for ( var i = 0; i < list.length; i++) {
            ...
            cont = cont + "<td><div class='share share_size_large share_type_twitter'>" +
                "<button id='updateButtonId' class='share__btn' onclick = 'updateDept(" +
                list[i].id+
                ")'>编辑</button>" +
                "</div>\n" +
                "<div class='share share_size_large share_type_gplus'>" +
                "<a class='share__btn' onclick = 'deleteDept(" +
                list[i].id+
                ")'>删除</a>" +
                "</div>" +
                "</td>" +
    ...
    $(document).ready(function() {
        ...
        $('#createButtonId').click(function() {
            ...
            $.ajax({
                url: "http://localhost:8001/dept/insertDept",
                contentType: "application/json;charset=UTF-8",
                type: "post",
                dataType: "json",
                data: formData,
        ...
        $('#updateButtonId').click(function() {
            ...
            $.ajax({
                url: "http://localhost:8001/dept/updateDept",
                contentType: "application/json;charset=UTF-8",
                type: "post",
                dataType: "json",
                data: formData,
    ...
    <!--前往更新页面-->
    function updateDept(primaryKey) {
        $(document).ready(function() {
            <!--新建列表-->
            $.ajax({
                url: "http://localhost:8001/dept/queryDept?id="+primaryKey,
                type: "get",
                success: function (data) {
    ...
    <!--删除操作-->
    function deleteDept(primaryKey) {
        $(document).ready(function() {
            <!--新建列表-->
            $.ajax({
                url: "http://localhost:8001/dept/deleteDept?id="+primaryKey,
                type: "get",
                success: function (data) {
                    if(data.success) {
                        alert("删除成功~~");

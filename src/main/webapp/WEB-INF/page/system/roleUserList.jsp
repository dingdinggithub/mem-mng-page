    ...
    <div class="graph-visual tables-main">
        <h4 class="inner-tittle">
            <div class="share share_size_large share_type_twitter">
                <a class="share__btn" href="#" data-toggle="modal" data-target="#createModal">新增用户角色关系</a>
...
<script type="text/javascript">
    ...
    function flushPage() {
        <!--初始化列表-->
        $.ajax({
            url : "http://localhost:8001/roleUser/queryPage",
    ...
    $(document).ready(function() {
        ...
        $('#createButtonId').click(function() {
            ...
            $.ajax({
                url: "http://localhost:8001/roleUser/insertRoleUser",
                contentType: "application/json;charset=UTF-8",
                type: "post",
                dataType: "json",
                data: formData,
        ...
        $('#updateButtonId').click(function() {
            ...
            $.ajax({
                url: "http://localhost:8001/roleUser/updateRoleUser",
                contentType: "application/json;charset=UTF-8",
                type: "post",
                dataType: "json",
                data: formData,
    ...
    <!--前往更新页面-->
    function updateRoleUser(primaryKey) {
        $(document).ready(function() {
            <!--新建列表-->
            $.ajax({
                url: "http://localhost:8001/roleUser/queryRoleUser?id="+primaryKey,
                type: "get",
                success: function (data) {
                    var dataModel = data.model;
                    $('#editId').val(dataModel.id);
                    $('#editRoleIdId').val(dataModel.roleId);
                    $('#editUserIdId').val(dataModel.userId);
    ...
    <!--删除操作-->
    function deleteRoleUser(primaryKey) {
        $(document).ready(function() {
            <!--新建列表-->
            $.ajax({
                url: "http://localhost:8001/roleUser/deleteRoleUser?id="+primaryKey,
                type: "get",
                success: function (data) {
                    if(data.success) {
                        alert("删除成功~~");
                                    ...
                                    <div class="form-body">
                                        <form class="form-horizontal" id="editFormlId">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-9">
                                                    <input name="userId" type="text" class="form-control" placeholder="userId" id="editUserIdId">
                                                </div>
                                            </div>
                                        </form>
                                    </div>
